...
\mbox{}\verb@}@\\
\mbox{}\verb@@\\
\mbox{}\verb@rfree1way <- function(n, delta = 0, offset = 0, alloc_ratio = 1, nblocks = 1, strata_ratio = 1, @\\
...
\mbox{}\verb@    ret$offset <- offset@\\
\mbox{}\verb@    if (B == 1L) ret$blk <- NULL@\\
\mbox{}\verb@    ret <- ret[rep(seq_len(nrow(ret)), times = N), , drop = FALSE]@\\
\mbox{}\verb@    ret$y <- .rfree1way(nrow(ret), delta = ret$offset + c(0, delta)[ret$trt], link = link)@\\
\mbox{}\verb@    return(ret)@\\
\mbox{}\verb@}@\\
\mbox{}\verb@@{\NWsep}
\end{list}
