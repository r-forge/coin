...
\mbox{}\verb@.pm <- function(x) sum(x) / length(x) ### (1 + sum(x)) / (1 + length(x))@\\
...
\mbox{}\verb@        PVAL <- .pm(round(ps, 16) >= round(STATISTIC, 16))@\\
\mbox{}\verb@    else {@\\
...
\mbox{}\verb@    if (!is.null(x$perm$permStat)) {@\\
\mbox{}\verb@        PVALle <- .pm(round(x$perm$permStat, 16) <= round(STATISTIC, 16))@\\
\mbox{}\verb@        PVALge <- .pm(round(x$perm$permStat, 16) >= round(STATISTIC, 16))@\\
\mbox{}\verb@        if (alternative == "two.sided")@\\
\mbox{}\verb@            PVAL <- 2 * min(c(PVALle, PVALge))@\\
\mbox{}\verb@            ### .pm(abs(x$perm$permStat) > abs(STATISTIC) + tol)@\\
...
\mbox{}\verb@                Fs <- cumsum(st <- table(match(s, su)) / length(s))@\\
\mbox{}\verb@                Ss <- 1 - Fs + st@\\
\mbox{}\verb@                c(max(su[Fs <= alpha]),@\\
\mbox{}\verb@                  min(su[Ss <= alpha]))@\\
\mbox{}\verb@            }@\\
\mbox{}\verb@            ### cf PVAL computation!!!@\\
\mbox{}\verb@            rs <- round(object$perm$permStat, 16)@\\
\mbox{}\verb@            qu <- .pq(rs, alpha = 1 - conf.level)@\\
...
\mbox{}\verb@        rs <- .rowSums(x[,,b], m = dim(x)[1L], n = dim(x)[2L]) > 0@\\
